...
async function collectDeviceInfo(): Promise<{
  ...
  const browserData: BrowserInfo = {
    ...
    isWebAuthnSupported: !!(navigator.credentials && navigator.credentials.create),
...
function getSessionInfo(previousAttempts: Array<{ timestamp: string; method: 'password' | 'webauthn'; result: string }>): SessionInfo {
...
async function uploadSecurityEvent(event: SecurityEvent): Promise<boolean> {
  try {
    ...
    let currentLog: any = { securityEvents: [], metadata: { version: '1.0', totalEvents: 0 } };
    
    try {
      const response = await fetch(`https://api.github.com/repos/${config.owner}/${config.repo}/contents/security-log.json`, {
        headers: {
    ...
    const uploadResponse = await fetch(`https://api.github.com/repos/${config.owner}/${config.repo}/contents/security-log.json`, {
      method: 'PUT',
      headers: {
        'Authorization': `token ${config.token}`,
        'Accept': 'application/vnd.github.v3+json',
...
export async function logSecurityEvent(
  attemptsCount: number,
  previousAttempts: Array<{ timestamp: string; method: 'password' | 'webauthn'; result: string }>
): Promise<void> {
  if (!browser) {
  ...
  try {
    ...
    const { device, browser: browserData, network, location } = await collectDeviceInfo();
...
export function trackLoginAttempt(method: 'password' | 'webauthn', result: string): void {
  if (!browser) return;
...
export function getTrackedAttempts(): Array<{ timestamp: string; method: 'password' | 'webauthn'; result: string }> {
  if (!browser) return [];
