...
export async function testGitHubConnection(config: GitHubConfig): Promise<GitHubSyncResult> {
  if (!browser) {
    return { success: false, error: 'GitHub sync only available in browser' };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { 
        success: false, 
        error: `Repository access failed: ${errorData.message || response.statusText}` 
      };
    }
    ...
    if (!repoData.private) {
      return { 
        success: false, 
        error: 'Repository must be private for security' 
      };
    }
