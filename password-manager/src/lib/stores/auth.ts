...
import { encrypt, decrypt, deriveKey, generateSalt, saltToString, stringToSalt, createVerificationObject, verifyPassword } from '$lib/utils/crypto';
...
export const initializeVault = async (password: string): Promise<EncryptedVault> => {
  if (!browser) {
  ...
  const { ciphertext: encryptedVerification, nonce: verificationNonce } = encrypt(verificationObj, key);
...
export const unlockVault = async (password: string, encryptedVault: EncryptedVault): Promise<boolean> => {
  if (!browser) {
    throw new Error('Vault operations can only be performed in the browser');
  }
