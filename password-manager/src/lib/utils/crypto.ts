...
export const encrypt = (data: string, key: Uint8Array): { ciphertext: string, nonce: string } => {
  if (!browser) {
    throw new Error('Crypto operations can only be performed in the browser');
  }
...
export const decrypt = (ciphertext: string, nonce: string, key: Uint8Array): string | null => {
  if (!browser) {
    throw new Error('Crypto operations can only be performed in the browser');
  }
